"""
01b_sensitivity_ex91.py — Sensitivity analysis: ICT footprint EXCLUDING sector 91
==================================================================================
Identical to 01_compute_ict_footprint.py except:
  - Sector 91 (Electrical equipment, ISIC C27) is EXCLUDED from ICT_SECTORS
  - All output parquet filenames carry an _ex91 suffix

Purpose: Test whether the reported patterns (geographic asymmetry, copper dominance,
temporal trajectory) are robust to the exclusion of the broadest ICT sector.
...
OUTPUT_DIR = _REPO / "data" / "sensitivity_ex91"
...
# SENSITIVITY: sector 91 (Electrical equipment, ISIC C27) EXCLUDED
ICT_SECTORS = {
    90: "Computers, electronics, optical instruments",
    # 91: excluded — Electrical equipment (ISIC C27) too broad
    109: "Publishing",
...
def process_year(year, ind_full, prod_full, full_to_ind, full_to_prod,
    ...
    print(f"\n{'=' * 70}")
    print(f"  PROCESSING YEAR {year} [SENSITIVITY: ex91]")
    print(f"{'=' * 70}")
...
def main():
    ...
    print("=" * 70)
    print("  ICT MATERIAL FOOTPRINT — SENSITIVITY ANALYSIS (EXCLUDING SECTOR 91)")
    ...
    print(f"  ICT sectors (ex91): {ICT_SECTOR_IDS}")
    ...
    for year in years_to_process:
        ...
        if mult_df is not None:
            ...
            # Save with _ex91 suffix
            mult_df.to_parquet(OUTPUT_DIR / f"ict_multipliers_{year}_ex91.parquet",
                               index=False)
            foot_df.to_parquet(OUTPUT_DIR / f"ict_footprint_{year}_ex91.parquet",
                               index=False)
            print(f"  Saved: ict_multipliers_{year}_ex91.parquet, ict_footprint_{year}_ex91.parquet")

    if all_mult:
        combined_mult = pd.concat(all_mult, ignore_index=True)
        combined_foot = pd.concat(all_foot, ignore_index=True)
        combined_mult.to_parquet(OUTPUT_DIR / "ict_multipliers_all_ex91.parquet",
                                 index=False)
        combined_foot.to_parquet(OUTPUT_DIR / "ict_footprint_all_ex91.parquet",
                                 index=False)
        print(f"\n  Combined multipliers (ex91): {len(combined_mult):,} rows")
        print(f"  Combined footprints (ex91): {len(combined_foot):,} rows")
    ...
    print("\n" + "=" * 70)
    print("  SENSITIVITY PIPELINE COMPLETE (ex91)")
    print("=" * 70)
